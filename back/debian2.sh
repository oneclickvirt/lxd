...
lxc exec "$1" -- curl -L https://raw.githubusercontent.com/oneclickvirt/lxd/main/ssh.sh -o ssh.sh
...
lxc config device add "$1" nat-ports proxy listen=tcp:0.0.0.0:"$4"-"$5" connect=tcp:127.0.0.1:"$4"-"$5" nat=true
echo "$2"
rm -rf "$0"
