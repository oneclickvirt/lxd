...
# lxc exec "$name" -- curl -L https://raw.githubusercontent.com/oneclickvirt/lxd/main/scripts/ssh.sh -o ssh.sh
...
# lxc exec "$name" -- curl -L https://raw.githubusercontent.com/oneclickvirt/lxd/main/scripts/config.sh -o config.sh
...
if [ -n "$9" ]; then
  if [ "$9" == "Y" ]; then
    if [ ! -f "./build_ipv6_network.sh" ]; then
      ...
      curl -L https://raw.githubusercontent.com/oneclickvirt/lxd/main/scripts/build_ipv6_network.sh -o build_ipv6_network.sh && chmod +x build_ipv6_network.sh > /dev/null 2>&1
    fi
